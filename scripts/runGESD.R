gesd <- function(obs, alpha, value.zscore=NA, r=NA) {
	...
	for(i in 1:r){
		...
		if((value.zscore == "YES") | (value.zscore == "Y")){
			...
			z <- abs(obs_new - mean(obs_new, na.rm=T))/sd(obs_new, na.rm=T) # Z-scores
		} else{
			print("ERROR! Inappropriate value for value.score=[YES|NO]")
		}
		
		max_ind <- which(z==max(z, na.rm=T),arr.ind=T)[1] # in case of ties, return first one
	...
	num_outliers <- sum(outlier_rank != 0, na.rm=T) #14 and 25 missing
